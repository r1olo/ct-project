...
export default function validateProg(prog: Prog): Analysis | never {
    /* for now, only check for undefined variables */
    let graph = genGraph(prog.cmd);
    let defVarsMap = analyzeDefinedVars(graph, prog.in);
    let valid = checkUndefined(defVarsMap);

    /* throw if some variable is undefined */
    if (!valid.ok)
        throw valid.err;

    /* return the rest of the maps */
    return {
        in:           prog.in,
        out:          prog.out,
        graph,
        definedVars:  defVarsMap,
